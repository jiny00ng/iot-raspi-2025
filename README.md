...
라즈베리파이를 활용한 **물류창고 온·습도 실시간 모니터링 시스템**입니다.  
설정 기준치를 초과할 경우 **경고음 및 LED 경고**를 제공하며,  
측정된 모든 데이터를 **DB에 저장**해 사용자 웹 대시보드에서 확인할 수 있습니다.
...
- 💡 LED 상태 표시:
  - 기준치 이내: **Green LED 점등**
  - 기준치 초과: **Red ↔ Blue LED 깜빡임**
- 🌐 웹 대시보드에서 실시간 데이터 확인
- 🕒 측정 데이터 DB 저장 및 이력 조회
- ⚙️ 사용자 설정 기준치 변경 가능
...
| DHT22 센서       | 정밀 온습도 측정 센서            |
| 부저 모듈        | 경고음을 위한 액티브 부저         |
| RGB LED 또는 개별 R/B/G LED | 상태 표시용             |
| 브레드보드 / 점퍼선 | 회로 구성용                   |
...
├── app.py                # Flask 웹 서버 + 센서 제어
├── sensor.py             # 센서 읽기 및 LED/부저 제어
